...
class pack {
  ...
  outPutScript () {
    ...
    if (this.animationList.size > 0) {
      ...
      this.outPutScriptData += Tool.loadFile(path.join(this.corePath, 'switchPage' , 'animation.js'))
    } else {
      log.debug('页面中不包含切换动画!')
      this.outPutScriptData += Tool.loadFile(path.join(this.corePath, 'switchPage' , 'noAnimation.js'))
    }
    // 写出主要硬盘文件
  ...
  handleScript (dom) {
    return new Promise((resolve, reject) => {
      ...
      // 处理动画效果
      this.extractAnimate(this.outPutScriptData)
      ...
      // 处理使用到的方法
      this.outPutScriptData += this.outPutTool(dom.script) + '\r\n'
      ...
      if (this.config.pageList.length === 1) {
        ...
        // 页面切换特效
        // 处理使用到的特效
        let useAnimationList = Tool.cutStringArray(this.outPutScriptData, 'owo.go(', ')').concat(Tool.cutStringArray(this.htmlTemple, 'owo.go(', ')'))
  ...
  handleStyle(dom) {
    return new Promise((resolve, reject) => {
      ...
      if (this.config.resourceFolder) {
        outPutCss = resourceHandle(outPutCss, this.config, '../', this.staticPath)
