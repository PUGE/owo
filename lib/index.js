  ...
  handleScript (dom) {
    return new Promise((resolve, reject) => {
      ...
      temp.forEach(condition => {
        ...
        let replacrStr = `/* if="${condition}" */`
        let content = ''
        if (this.outPutScriptData.indexOf(`/* end="${condition}" */`) > 0) {
          content = Tool.cutStringArray(this.outPutScriptData, replacrStr, `/* end="${condition}" */`)
          replacrStr = replacrStr + content + `/* end="${condition}" */`
        } else {
          replacrStr = replacrStr + Tool.cutStringArray(this.outPutScriptData, replacrStr, `/* end */`) + `/* end */`
        }
