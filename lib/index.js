...
class pack {
  ...
  handleScript (dom, changePath) {
    return new Promise((resolve, reject) => {
      ...
      // 处理js中的资源
      dom.script = resourceHandle(dom.script, this.config, this.basePath, this.staticPath)
      
      // 使用bable处理代码
      let mainScript = loadCoreCode(dom.script)
        ...
        if (changePath === undefined || changePath === fromPath) {
          ...
          if (element.resource) {
            ...
            fs.readFile(fromPath, (err, fileData) => {
              ...
              if (err) throw err
              fileData = resourceHandle(fileData.toString(), this.config, this.basePath, this.staticPath)
  ...
  handleStyle(dom) {
    return new Promise((resolve, reject) => {
      ...
      if (this.config.resourceFolder) {
        ...
        outPutCss = resourceHandle(outPutCss, this.config, this.basePath, this.staticPath)
  ...
  outPutHtml () {
    ...
    log.debug('准备输出html!')
    // 对html所引用的资源进行处理
    this.htmlTemple = resourceHandle(this.htmlTemple, this.config, this.basePath, this.staticPath)
    // 如果是debug模式那么不需要进行优化
