...
if (!fs.existsSync(path.join(__dirname, '../lib/index.html')) && fs.existsSync(path.resolve(process.argv[1], '../lib/index.html'))) {
  __dirname = path.resolve(process.argv[1], '../lib')
} else if (fs.existsSync(path.join(__dirname, './node_modules/@owo/owo/lib/index.html'))) {
  __dirname = path.join(__dirname, './node_modules/@owo/owo/lib/')
}
...
class pack {
  ...
  pack(updata) {
    ...
    // 读取入口模板文件(一次性读取到内存中)
    log.info(`读取内置模板文件`)
    this.htmlTemple = fs.readFileSync(path.resolve(__dirname, './index.html'), 'utf8')
