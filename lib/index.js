    ...
    if (!changePath) {
      ...
      if (watcherFileItem) {
        ...
        if (watcherFileItem.type === 'resource') {
          ...
          register.otherCSSFile(watcherFileItem, this.config)
          log.info('编译结束!')
          this.callBackInfo('end', '编译结束!')
          return
          ...
          // 插件做处理
          fileData = register.otherJSFile(fileData, watcherFileItem, this.config)
          // 输出路径
          const outPutFile = path.join(this.staticPath, 'js', `${watcherFileItem.name}.js`)
          fs.writeFile(outPutFile, fileData, (e) => {
            if (e) throw e
            log.debug(`处理文件: ${outPutFile}`)
          })
