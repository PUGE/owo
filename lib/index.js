...
class pack {
  ...
  outPutAnimate () {
    ...
    if (this.animateList.size === 0) {
      log.info('页面中没有动画效果!')
      log.info(this.animateList)
      this.htmlTemple = this.htmlTemple.replace(`<!-- animate-output -->`, '')
    } else {
      let animationData = ''
      const versionString = this.config.outPut.addVersion ? `.${this.version}` : ''
      ...
      this.htmlTemple = this.htmlTemple.replace(`<!-- animate-output -->`, `<link rel="stylesheet" href="${this.basePath}static/css/owo.animate${versionString}.css">`)
    }
  }
  ...
  handleScript (dom, changePath) {
    ...
    if (this.config.pageList.length === 1) {
      ...
      // 页面切换特效
      // 处理使用到的特效
      let useAnimationList = Cut.stringArray(dom.script, 'owo.go(', ')')
    ...
    // 无论多页面还是单页面都需要处理动画效果
    let useAnimateList = Cut.stringArray(dom.script, 'owo.tool.animate(', ',')
    ...
    // 数组去重
    toolList = new Set(toolList.concat(toolList))
    ...
    // 很傻 但是很有用
    // 判断是否使用owo.emit
    if (dom.script.includes('owo.emit(')) {
      coreScript += Tool.loadFile(path.join(this.corePath, 'emit.js'))
    } else {
      coreScript += Tool.loadFile(path.join(this.corePath, 'noEmit.js'))
    }
    // 判断是否使用了owo.change
    if (dom.script.includes('owo.change(')) {
      coreScript += Tool.loadFile(path.join(this.corePath, 'change.js'))
    }
  ...
  handleStyle(dom, changePath) {
    ...
    if (this.config.resourceFolder) {
      ...
      let pathStr = `../resource/`
      // 如果样式需要合并到html中 那么图片路径需要改变
      if (this.config.outPut.merge) {
        pathStr = `${this.basePath}static/resource/`
      }
      // console.log(outPutCss)
      outPutCss = resourceHandle(outPutCss, resourceFolder, path.join(this.staticPath, 'resource'), pathStr, this.config.outPut.embedSize)
    }
  ...
  pack(changePath) {
    ...
    this.callBackInfo('outPutHtml', '正在处理模块!')
    // 读取入口模板文件(一次性读取到内存中)
