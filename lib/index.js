...
class pack {
        ...
        if (element.src.startsWith('http')) {
          ...
          if (element.merge) {
            ...
            if (element.babel) {
              ...
              // 处理资源并移动
              log.info('开始处理附属script中的资源')
              this.resource.handleFile(this.config, fromPath, outPutFile, '../', element.resource)
            }
  ...
  handleScript (dom) {
    return new Promise((resolve, reject) => {
      ...
      dom.script = this.resource.handle(dom.script, this.config, this.basePath + 'static/')
  ...
  outOtherStyle (changePath) {
    return new Promise((resolve, reject) => {
      ...
      for (let ind = 0; ind < styleList.length; ind++) {
        ...
        const fromPath = path.normalize(path.join(runPath, element.src))
  ...
  handleStyle(dom) {
    return new Promise((resolve, reject) => {
      ...
      if (this.config.resourceFolder) {
        ...
        outPutCss = this.resource.handle(outPutCss, this.config, '../')
  ...
  outPutHtml () {
    ...
    log.debug('准备输出html!')
    // 对html所引用的资源进行处理
    this.htmlTemple = this.resource.handle(this.htmlTemple, this.config, this.basePath + 'static/')
    // 如果是debug模式那么不需要进行优化
    ...
    if (!changePath) {
      ...
      if (watcherFileItem) {
        ...
        if (watcherFileItem.type === 'resource') {
          this.resource.miniImg([changePath], path.join(this.staticPath, 'resource'))
          log.info('编译结束!')
          this.callBackInfo('end', '编译结束!')
          return
        } else if (watcherFileItem.type === 'css') {
          this.resource.handleFile(this.config, changePath, path.join(this.staticPath, 'css', `${watcherFileItem.name}.css`), '../', watcherFileItem.resource)
          log.info('编译结束!')
          this.callBackInfo('end', '编译结束!')
          return
        }
