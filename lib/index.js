...
class pack {
  ...
  outPutAnimation () {
    ...
    if (this.config.outPut.allAnimation || this.animationList.size > 0) {
      let animationData = ''
      const versionString = this.config.outPut.addVersion ? `.${this.version}` : ''
      // 判断是全部输出还是
      if (this.config.outPut.allAnimation) {
        animationData = Tool.loadFile(path.join(this.corePath, 'animation', `animations.css`)) + '\r\n'
      } else {
  ...
  multiScriptHandle (scriptData) {
    ...
    // 模板插值处理
    const temp = Tool.cutStringArray(coreScript, '/* if=', 'end */')
    temp.forEach(tempItem => {
      const condition = Tool.cutString(tempItem, '"', '"')
      // console.log(this.config)
      let valueItem = this.config
      let argList = condition.split('.')
      for (let ind = 0; ind < argList.length; ind++) {
        const element = argList[ind]
        if (valueItem[element]) {
          valueItem = valueItem[element]
        } else {
          valueItem = false
          break
        }
      ...
      if (valueItem) {
        let tempItemCopy = tempItem
        tempItemCopy = tempItemCopy.replace(`"${condition}"`, ``)
        coreScript = coreScript.replace(`/* if=${tempItem}end */`, tempItemCopy)
      } else {
        coreScript = coreScript.replace(`/* if=${tempItem}end */`, '')
      }
    })
    // 多页面才可能存在切换以及切换动画
    // 页面切换特效
    // 处理使用到的特效
    let useAnimationList = Tool.cutStringArray(scriptData, 'owo.go(', ')')
    // 遍历特效函数
    ...
    if (this.animationList.size > 0) {
      ...
      coreScript += Tool.loadFile(path.join(this.corePath, 'switchPage' , 'animation.js'))
    } else {
      log.debug('页面中不包含切换动画!')
      coreScript += Tool.loadFile(path.join(this.corePath, 'switchPage' , 'noAnimation.js'))
    }
    return coreScript
  }
  // 分析并加载必要的代码
  loadCoreCode (scriptData) {
    ...
    if (this.config.scheme) {
      this.config.scheme.forEach(element => {
        ...
        code += '\r\n' + Tool.loadFile(path.join(__dirname, '../scheme', `${element}.js`))
      })
    }
    return code
  }
