...
class pack {
  ...
  loadCoreCode (scriptData) {
    let code = '\r\n'
    // 判断是否使用owo.emit
    if (scriptData.includes('owo.emit(')) {
      code += Tool.loadFile(path.join(this.corePath, 'emit.js'))
    } else {
      code += Tool.loadFile(path.join(this.corePath, 'noEmit.js'))
    }
    // 判断是否使用了owo.change
  ...
  handleScript (dom, changePath) {
    ...
    if (this.config.pageList.length === 1) {
      coreScript += this.singleScriptHandle()
    } else {
      coreScript += this.multiScriptHandle(dom)
    }
    // 无论多页面还是单页面都需要处理动画效果
    ...
    // 处理使用到的方法
    coreScript += this.outPutTool(dom.script)
    // 加载必要插件
    coreScript += this.loadCoreCode(dom.script)
    ...
    // 处理引用的script
    scriptData += this.handOtherScript(changePath)
