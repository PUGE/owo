...
class pack {
  ...
  loadCoreCode (scriptData) {
    ...
    // 模板插值处理
    const temp = Tool.cutStringArray(code, '/* if=', 'end */')
    ...
    temp.forEach(tempItem => {
      ...
      if (valueItem) {
        code = code.replace(`/* if=${tempItem}end */`, tempItem.replace(`"${condition}"`, ``))
      } else {
        code = code.replace(`/* if=${tempItem}end */`, '')
      }
    })
    return code
  }
  outOtherScript (changePath) {
    return new Promise((resolve, reject) => {
      ...
      for (let ind = 0; ind < this.config.scriptList.length; ind++) {
        ...
        if (changePath !== undefined && changePath !== path.join(runPath, element.src)) {continue}
        // 如果是网络地址那么不需要进行处理
