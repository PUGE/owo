      ...
      if (selectors) {
        for (let selectorIndex = 0; selectorIndex < selectors.length; selectorIndex++) {
          if (selectors[selectorIndex].startsWith(`.${templeName} `)) {
            style.stylesheet.rules[ruleIndex].selectors[selectorIndex] = selectors[selectorIndex] + `.page-${templeName} "]`
          } else {
            // console.log(templeName)
            style.stylesheet.rules[ruleIndex].selectors[selectorIndex] = `.page-${templeName} ` + selectors[selectorIndex]
          }
        }
      } else {
