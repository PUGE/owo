...
function styleHandle (htmlText, boxClass) {
  // 是否需要预处理css
  ...
  for (let ruleIndex = 0; ruleIndex < style.stylesheet.rules.length; ruleIndex++) {
    ...
    if (selectors) {
      for (let selectorIndex = 0; selectorIndex < selectors.length; selectorIndex++) {
        if (selectors[selectorIndex].startsWith(`.${boxClass}`)) {
          style.stylesheet.rules[ruleIndex].selectors[selectorIndex] = selectors[selectorIndex]
        } else {
          style.stylesheet.rules[ruleIndex].selectors[selectorIndex] = `.${boxClass} ` + selectors[selectorIndex]
        }
      }
    } else {
