...
function getFileContent (pageInfo) {
  ...
  if (pageInfo.src.startsWith('http')) {
    // 模块下载目录
    let modulesPath = path.join(process.cwd(), 'owo_modules', fileName)
    // 统一路径为左斜线
    ...
    Storage.watcherFile[modulesPath] = {
      name: pageInfo.name,
      type: pageInfo.type
    }
  } else {
    // body目录
    let bodyFilePath = path.join(process.cwd(), pageInfo.src)
    // 统一路径为左斜线
    bodyFilePath = bodyFilePath.replace(/\\/g, '/')
    // console.log(bodyFilePath)
    if (fs.existsSync(bodyFilePath)) {
      headFileContent = fs.readFileSync(bodyFilePath, 'utf8')
      // 记录处理过的模板
      Storage.watcherFile[bodyFilePath] = {
        name: pageInfo.name,
        type: pageInfo.type
      }
    } else {
...
function scriptHandle (pageInfo, scriptCode, moduleScript) {
  ...
  switch(pageInfo.type) {
    ...
    case 'block': {
      ...
      moduleScript[pageName]._type = pageInfo.type
    ...
    case 'plug': {
      ...
      moduleScript.template[pageName]._type = pageInfo.type
...
function pageHandle (templet, config) {
  ...
  // 全局变量初始化
  [scriptData, styleText, serverTempletMap, Config, htmlCache, srcCache, fileCache, fontList, hoverList, globalScript] = [{}, '', {}, config, {}, [], {}, {}, new Set(), '']
  ...
  // 转换成纯文本
  scriptData = Tool.fnStringify(scriptData)
  // 处理js中的链接
  if (config.handleLink) {
    // console.log(scriptData)
    scriptData = handleLink(scriptData, path.join(process.cwd(), config.outFolder) + '/static/resource/', './static/resource/')
  }
  let returnScriptData = `${globalScript}\r\nowo.script = ${scriptData}`
