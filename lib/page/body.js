...
function templateTextHandle (pageInfo, headFileContent, pageScript, scriptCode) {
  ...
  switch (body.children.length) {
    ...
    case 1: {
      ...
      domBox.classList.add(`page-${pageName}`)
      for (let ind = 0; ind < domBox.classList.length; ind++) {
        ...
        if (Config.outPut.minifyCss) {
          needReplaceCssList.push([`.page-${pageName} .${classString}{`, `.${classString}.page-${pageName}{`])
        } else {
          // 修复外层盒子class规则
          needReplaceCssList.push([`.page-${pageName} .${classString}:`, `.${classString}.page-${pageName}:`])
          needReplaceCssList.push([`.page-${pageName} .${classString} `, `.${classString}.page-${pageName} `])
      ...
      // 修复外层盒子标签规则
      if (Config.outPut.minifyCss) {
        needReplaceCssList.push([`.page-${pageName} ${domBox.localName}{`, `${domBox.localName}.page-${pageName}{`])
      } else {
        needReplaceCssList.push([`.page-${pageName} ${domBox.localName}:`, `${domBox.localName}.page-${pageName}:`])
        needReplaceCssList.push([`.page-${pageName} ${domBox.localName} `, `${domBox.localName}.page-${pageName} `])
        needReplaceCssList.push([`.page-${pageName} ${domBox.localName}{`, `${domBox.localName}.page-${pageName}{`])
