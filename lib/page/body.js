...
function pageHandle (templet, config) {
  ...
  for (let ind = 0; ind < config.pageList.length; ind++) {
    ...
    const templeTemp = `<temple name="${element.name}" src="${element.src}" type="page"></temple>`
    templeData += `\r\n    <!-- 页面[${element.name}]-->\r\n    ${templeTemp}`
  ...
  // 插件处理
  if (config.plugList) {
    for (let ind = 0; ind < config.plugList.length; ind++) {
    
      const element = config.plugList[ind]
      const templeTemp = `<temple name="${element.name}" src="${element.src}" type="plug"></temple>`
      templeData += `\r\n    <!-- 插件[${element.name}]-->\r\n    ${templeTemp}`
      config.plugList[ind].temple = templeTemp
      // pageList均为页面，所以需要添加上页面标识
      config.plugList[ind].type = 'plug'
    }
  }
  // console.log(templet)
  ...
  // ---------------------- 插件处理 ----------------------
  if (config.plugList) {
    config.plugList.forEach(pageInfo => {
      pageInfo.outerHTML = templet
      pageInfo.hash = crypto.createHash('md5').update(templet).digest('hex')
      const startTime = (new Date()).valueOf()
      templet = htmlTextHandle(pageInfo)
