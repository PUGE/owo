...
function funHandle (dom, scriptData, pageInfo) {
  ...
  switch (dom.tagName) {
    ...
    case 'GO': {
      let attributeStr = ` class="owo-go ${dom.classList.value}"`
      for (const key in prop) {
        const element = prop[key];
        attributeStr += ` ${key}="${element}"`
      }
      // 判断是否有动画
      if (prop.ani) {
        const aniList = prop.ani.split('/')
        aniList.forEach(element => {
...
function htmlTextHandle (pageInfo, htmlText, moduleScript) {
  ...
  // 模板插值处理
  htmlCache[pageInfo._hash] = interpolationHandle(htmlText, pageScript)
  return htmlCache[pageInfo._hash]
}
