        ...
        if (attribute.value.includes('$go(')) {
          ...
          if (parameterList[1]) {
            ...
            temp.forEach(animation => {
              useAnimationList.add(animation)
            })
          }
          if (parameterList[2]) {
            // 一个动画里面可能有多个动画
  ...
  switch (body.children.length) {
    ...
    case 1: {
      ...
      for (let ind = 0; ind < domBox.classList.length; ind++) {
        ...
        needReplaceCssList.push([`.ox-${pageInfo.name} .${classString} `, `.ox-${pageInfo.name} `])
      }
    ...
    default: {
      // 给元素增加页面专属的class和id
      const domBox = document.createElement("session")
      domBox.classList.add(`ox-${pageInfo.name}`)
      // 如果是页面则为其加上id和class
      if (pageInfo.isPage) {
        // console.log(pageName, newTemplateName)
...
function pageHandle (templet, config) {
  // 全局变量初始化
  [scriptData, styleText, serverTempletMap, Config, needReplaceCssList] = [{}, '', {}, config, []]
  ...
  log.debug(`页面中使用到的动画列表: ${useAnimationList.toString()}`)
  return {
    html: templet,
    style: styleText,
    script: `
