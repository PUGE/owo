...
function scriptHandle (pageInfo, scriptCode, moduleScript) {
  ...
  switch(pageInfo.type) {
    ...
    case 'route': {
      ...
      // 判断是否为模板
      // 待修复 先以对象方式添加吧
      if (moduleScript.view === undefined) moduleScript.view = {}
      
      if (moduleScript.view[viewName] === undefined) moduleScript.view[viewName] = []
      let temp = scriptCode
...
function templateTextHandle (pageInfo, headFileContent, pageScript, scriptCode, assemblyName, htmlText) {
  // 获取到页面/模块名
  const templeName = pageInfo.type !== 'plug' ? pageInfo.name : pageInfo.templeName
  const pageSrc = pageInfo.type !== 'plug' ? pageInfo.src : (pageInfo.prop['_src'] || pageInfo.prop['src'])
  ...
  if (!srcCache.includes(assemblyName + pageSrc)) {
    ...
    styleText += '\r\n' + Style(headFileContent, domBox.classList, domBox.localName, pageInfo.type !== 'plug' && Config.pageList.length < 2)
  } else {
...
function htmlTextHandle (pageInfo, htmlText, moduleScript) {
  ...
  // 页面或者模块的名称
  let assemblyName = pageInfo.type !== 'plug' ? pageInfo.name : pageInfo.templeName
  const assemblyNameCopy = assemblyName
  // console.log(scriptData, pageInfo.name)
  let scriptDataTemp = pageInfo.type !== 'plug' ? scriptData : scriptData[pageInfo.name].template
  ...
  if (scriptDataTemp) {
    ...
    while (assemblyName in scriptDataTemp) {
      ...
      assemblyName = `${pageInfo.type !== 'plug' ? pageInfo.name : pageInfo.templeName}-${pageNameIndex}`
    }
    
    // 判断是否被更名过
    if (pageNameIndex > 0) {
      log.debug(`模块 ${pageInfo.type !== 'plug' ? pageInfo.name : pageInfo.templeName} 因名称重复被重命名为 ${assemblyName}`)
