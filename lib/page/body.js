...
// 待优化 可以缓存的更好
function scriptHandle (pageInfo, pageName, scriptCode) {
  if (scriptCode) {
    // 清理无用对象
  ...
  if (pageInfo.isPage) {
    ...
    return scriptData[pageInfo.pageName].template[pageName]
  }
}

function templateTextHandle (pageInfo, headFileContent, pageScript, scriptCode, pageName) {
  const pageSrc = pageInfo.src || pageInfo.prop.src
  // 获取模板的html文本
...
function htmlTextHandle (pageInfo) {
  ...
  let pageNameIndex = 0
  // 取出页面或组件缓存
  let scriptDataTemp = pageInfo.isPage ? scriptData : scriptData[pageInfo.pageName].template
  if (scriptDataTemp) {
  ...
  if (htmlCache[pageInfo.hash]) {
    log.debug(`使用缓存返回模板: ${pageName}`)
    scriptHandle(pageInfo, pageName, scriptCache[pageInfo.hash])
  ...
  // 存储当前页面的
  let pageScript = scriptHandle(pageInfo, pageName, scriptCode)
  // console.log(scriptText)
  // 如果页面没有js也为其创建一个空对象
  ...
  htmlText = templateTextHandle(pageInfo, headFileContent, pageScript, scriptCode, pageName)
