...
import checkNoScript from '../tool/checkNoScript.js'
// 同步请求
...
function getFileContent (pageInfo) {
  ...
  if (pageInfo.src.startsWith('http')) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      headFileContent = request('GET', pageInfo.src).getBody('utf8')
      // 注册下载完毕事件
      ...
      if (headFileContent) {
        ...
        throw new Error('模块: ' + pageInfo.src + '下载失败!')
      }
...
function htmlTextHandle (pageInfo, htmlText, moduleScript) {
  ...
  if (scriptText !== null) {
    ...
    let ast = null
    try {
      ast = babelParse.parse(scriptText, {sourceType: "module"});
    } catch (error) {
      throw Error(`位置 ${scriptText.substring(error.pos - 10, error.pos + 10)} 处语法错误!`)
    }
    let pageCode = ''
    ...
    if (pageCode) {
      ...
      try {
        // 兼容module.exports模式
        pageCode = pageCode.replace(/module.exports.*=/, 'return ')
        scriptCode = new Function(pageCode)()
      } catch (error) {
        throw error
      }
    }
...
function pageHandle (templet, config) {
  ...
  if (config.handleLink) {
    ...
    scriptDataText = handleLink(scriptDataText, path.join(process.cwd(), config.outFolder) + '/static/images/', './static/images/')
  }
  ...
  // 判断是否可以精简掉core.js
  let noScript = checkNoScript(scriptData)
  // 多页网站不能精简
  if (config.pageList.length > 1) {
    noScript = false
  }
