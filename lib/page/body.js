...
function funHandle (dom, scriptData, pageInfo) {
  ...
  for (let ind = 0; ind < dom.attributes.length; ind++) {
    ...
    if (value.startsWith('{{') && value.endsWith('}}')) {
      propMap[nodeName] = value.slice(2, -2)
      // 变量参数处理
  ...
  if (dom.tagName === 'PLUG') {
    ...
    dom.outerHTML = `<div view="${templeName}">${dom.innerHTML}</div>`
  } else if (dom.tagName === 'ROUTE') {
    // console.log(prop)
    // 处理模板
    const startTime = (new Date()).valueOf()
    dom.outerHTML = htmlTextHandle({
    ...
    log.debug(`路由${prop['_name'] || prop.name}处理用时: ${(new Date()).valueOf() - startTime} 毫秒!`)
    return
  }
  
  if (dom.attributes.length > 0) {
    let prop = dom.attributes
    // 待优化不
    // 读取DOM的每一个属性
      ...
      switch (attribute.name) {
        ...
        case 'o-animation': {
          ...
          if (!animationList.has(value)) {
            const filePath = path.join(__dirname, '../../', 'core' , 'animation2', `${value}.css`)
    ...
    // 单独处理:for 因为如果不对:for会改变DMO结构 ，并且:for复制的dom节点都是相同的
    prop = dom.attributes
    // 待优化不
    // 读取DOM的每一个属性
    for (let i = 0; i < prop.length; i++) {
...
function scriptHandle (pageInfo, scriptCode, moduleScript) {
  ...
  const pageName = pageInfo.type === 'page' ? pageInfo.name : pageInfo.templeName
  ...
  switch(pageInfo.type) {
    ...
    case 'plug': {
      ...
      moduleScript.template[pageName].propMap = pageInfo.propMap
      return moduleScript.template[pageName]
    }
    case 'route': {
      const viewName = pageInfo.viewName
      const routeName = pageInfo.prop._name || pageInfo.prop.name
      // 判断是否为模板
      // 待修复 先以对象方式添加吧
      if (moduleScript.route === undefined) moduleScript.route = {
        _name: viewName,
        _show: function (name) {
          document.querySelector('[view="' + this._name +'"] [route="' + name +'"]').style.display = 'block'
        }
      }

      if (moduleScript.route[viewName] === undefined) moduleScript.route[viewName] = []
      let temp = scriptCode
      temp.prop = pageInfo.prop
      temp._name = routeName
      temp.propMap = pageInfo.propMap
      moduleScript.route[viewName].push(temp)
      const itemIndex = moduleScript.route[viewName].length - 1
      return moduleScript.route[viewName][itemIndex]
    }
  }
}
...
function templateTextHandle (pageInfo, headFileContent, pageScript, scriptCode, assemblyName, htmlText) {
  // 获取到页面/模块名
  const templeName = pageInfo.type === 'page' ? pageInfo.name : pageInfo.templeName
  const pageSrc = pageInfo.type === 'page' ? pageInfo.src : (pageInfo.prop['_src'] || pageInfo.prop['src'])
  ...
  // console.log(pageScript)
  if (pageInfo.type === 'page' || Object.keys(pageScript).length > 1) {
...
function htmlTextHandle (pageInfo, htmlText, moduleScript) {
  ...
  // 页面或者模块的名称
  let assemblyName = pageInfo.type === 'page' ? pageInfo.name : pageInfo.templeName
  const assemblyNameCopy = assemblyName
  // console.log(assemblyName)
  let scriptDataTemp = pageInfo.type === 'page' ? scriptData : scriptData[pageInfo.name].template
  // console.log(scriptDataTemp)
  if (scriptDataTemp) {
    
    while (assemblyName in scriptDataTemp) {
      pageNameIndex++
      // console.log(pageInfo.type === 'page' ? pageInfo.name : pageInfo.templeName)
  ...
  const scriptCode = scriptText ? eval(scriptText) : null
  // 判断缓存中是否有此模板,有的话不需要重新计算
  if (!scriptCode || !scriptCode.data) {
    if (htmlCache[pageInfo.hash]) {
      log.debug(`使用缓存返回模板: ${assemblyName}`)
      scriptHandle(pageInfo, scriptCache[pageInfo.hash], moduleScript)
  ...
  return {
    ...
    script: `
      /*
        ...
        键值：方法列表
      */
      owo.phoneEnter = "${Config.phoneEnter ? Config.phoneEnter : null}"
