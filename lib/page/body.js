...
function funHandle (dom, scriptData, pageInfo) {
  ...
  if (dom.tagName === 'TEMPLE') {
    ...
    // 取出模板属性列表列表
    let prop = {}
    for (let ind = 0; ind < dom.prop.length; ind++) {
      const nodeName = dom.prop[ind].nodeName
      let value = dom.prop[ind].value
      // 判断是一个纯文本字符串还是一个变量
...
function htmlTextHandle (pageInfo, isTemple) {
  ...
  if (scriptText) {
    ...
    log.error(`模块: ${pageName} 不包含script`)
    // 页面不包含attributes所以要进行判断
    scriptData[pageName] = {
      prop: pageInfo.prop ? pageInfo.prop : {}
    }
    pageScript = scriptData[pageName]
  }
  // 判断是否需要使用插件预处理
  ...
  switch (body.children.length) {
    ...
    case 1: {
      ...
      // 防止样式重复打包
      if (!srcCache.includes(pageSrc)) {
        srcCache.push(pageSrc)
        // 模板插值处理
        headFileContent = interpolationHandle(headFileContent, pageScript)
        // 解析样式
        // 新一批的样式需要重启一行
        styleText += '\r\n' + Style(headFileContent, domBox.classList[0])
      } else {
        log.debug(`智能跳过打包${pageSrc}的重复样式!`)
      }
      break
    }
    default: {
...
function pageHandle (templet, config) {
  ...
  // 全局变量初始化
  [scriptData, styleText, serverTempletMap, Config, needReplaceCssList, htmlCache, srcCache, fileCache] = [{}, '', {}, config, [], {}, [], {}]
