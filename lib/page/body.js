...
function templateTextHandle (pageInfo, headFileContent, pageScript, scriptCode, assemblyName) {
  ...
  switch (body.children.length) {
    ...
    case 1: {
      ...
      domBox.classList.add(`owo-${assemblyName}`)
      for (let ind = 0; ind < domBox.classList.length; ind++) {
        ...
        if (Config.outPut.minifyCss) {
          needReplaceCssList.push([`.owo-${assemblyName} .${classString}{`, `.${classString}.owo-${assemblyName}{`])
        } else {
          // 修复外层盒子class规则
          needReplaceCssList.push([`.owo-${assemblyName} .${classString}:`, `.${classString}.owo-${assemblyName}:`])
          needReplaceCssList.push([`.owo-${assemblyName} .${classString} `, `.${classString}.owo-${assemblyName} `])
      ...
      // 修复外层盒子标签规则
      if (Config.outPut.minifyCss) {
        needReplaceCssList.push([`.owo-${assemblyName} ${domBox.localName}{`, `${domBox.localName}.owo-${assemblyName}{`])
      } else {
        needReplaceCssList.push([`.owo-${assemblyName} ${domBox.localName}:`, `${domBox.localName}.owo-${assemblyName}:`])
        needReplaceCssList.push([`.owo-${assemblyName} ${domBox.localName} `, `${domBox.localName}.owo-${assemblyName} `])
        needReplaceCssList.push([`.owo-${assemblyName} ${domBox.localName}{`, `${domBox.localName}.owo-${assemblyName}{`])
