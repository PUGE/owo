...
function funHandle (dom, scriptData, pageInfo) {
  ...
  // const pageName = pageInfo.isPage ? pageInfo.name : pageInfo.prop['_name']
  // 模板处理
...
function templateTextHandle (pageInfo, headFileContent, pageScript, scriptCode, assemblyName) {
  ...
  const pageSrc = pageInfo.isPage ? pageInfo.src : (pageInfo.prop['_src'] || pageInfo.prop['src'])
  ...
  for (const templateKey in scriptData) {
    if (scriptData.hasOwnProperty(templateKey)) {
      ...
      if (templateValue.template) {
        // 递归处理子模块
        clearData(templateValue.template)
      }
    }
  }
}
