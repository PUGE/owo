...
function scriptHandle (pageInfo, scriptCode = {}) {
  ...
  scriptData[pageName].type = pageInfo.type
  // 判断是页面还是模块
  if (pageInfo.type) {
...
function pageHandle (templet, config) {
  ...
  for (let ind = 0; ind < config.pageList.length; ind++) {
    ...
    // 判断是否为插件
    config.pageList[ind].type = config.pageList[ind].isPlug ? 'plug' : 'page'
    const templeTemp = `<temple name="${element.name}" src="${element.src}" type="${config.pageList[ind].type}"></temple>`
    templeData += `\r\n    <!-- 页面[${element.name}]-->\r\n    ${templeTemp}`
