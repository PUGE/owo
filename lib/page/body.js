...
function funHandle (dom, scriptData, pageInfo) {
  ...
  if (dom.tagName === 'TEMPLE') {
    ...
    log.debug(`模块${dom.attributes['name'].value}处理用时: ${(new Date()).valueOf() - startTime} 毫秒!`)
    return
...
function scriptHandle (pageInfo, scriptCode) {
  ...
  if (pageInfo.isPage) {
    ...
    // 给页面或者模块加上自己的名字
    scriptData[pageName].$name = pageName
    return scriptData[pageName]
  } else {
    // 判断模板是否包含created事件
    // 判断是否为模板
    if (scriptData[pageInfo.name] === undefined) scriptData[pageInfo.name] = {}
    ...
    // 给页面或者模块加上自己的名字
    scriptData[pageInfo.name].template[pageName].$name = pageName
 ...
 * @return {string} 返回参数值
 */
function templateTextHandle (pageInfo, headFileContent, pageScript, scriptCode, assemblyName) {
  // 获取到页面/模块名
  const templeName = pageInfo.isPage ? pageInfo.name : pageInfo.templeName
  ...
  switch (body.children.length) {
    ...
    case 1: {
      ...
      domBox.classList.add(`page-${assemblyName}`)
      for (let ind = 0; ind < domBox.classList.length; ind++) {
        ...
        if (Config.outPut.minifyCss) {
          needReplaceCssList.push([`.page-${assemblyName} .${classString}{`, `.${classString}.page-${assemblyName}{`])
        } else {
          // 修复外层盒子class规则
          needReplaceCssList.push([`.page-${assemblyName} .${classString}:`, `.${classString}.page-${assemblyName}:`])
          needReplaceCssList.push([`.page-${assemblyName} .${classString} `, `.${classString}.page-${assemblyName} `])
      ...
      // 修复外层盒子标签规则
      if (Config.outPut.minifyCss) {
        needReplaceCssList.push([`.page-${assemblyName} ${domBox.localName}{`, `${domBox.localName}.page-${assemblyName}{`])
      } else {
        needReplaceCssList.push([`.page-${assemblyName} ${domBox.localName}:`, `${domBox.localName}.page-${assemblyName}:`])
        needReplaceCssList.push([`.page-${assemblyName} ${domBox.localName} `, `${domBox.localName}.page-${assemblyName} `])
        needReplaceCssList.push([`.page-${assemblyName} ${domBox.localName}{`, `${domBox.localName}.page-${assemblyName}{`])
      ...
      if (!srcCache.includes(assemblyName + pageSrc)) {
        ...
        // 解析样式
        // 新一批的样式需要重启一行
        styleText += '\r\n' + Style(headFileContent, assemblyName)
      } else {
...
function htmlTextHandle (pageInfo) {
  ...
  // 页面或者模块的名称
  let assemblyName = pageInfo.isPage ? pageInfo.name : pageInfo.templeName
  ...
  if (scriptDataTemp) {
    ...
    while (assemblyName in scriptDataTemp) {
      pageNameIndex++
      // console.log(pageInfo.isPage ? pageInfo.name : pageInfo.templeName)
      assemblyName = `${pageInfo.isPage ? pageInfo.name : pageInfo.templeName}-${pageNameIndex}`
    }
    
    // 判断是否被更名过
    if (pageNameIndex > 0) {
      log.debug(`模块 ${pageInfo.isPage ? pageInfo.name : pageInfo.templeName} 因名称重复被重命名为 ${assemblyName}`)
      if (pageInfo.isPage) pageInfo.name = assemblyName
  ...
  htmlText = templateTextHandle(pageInfo, headFileContent, pageScript, scriptCode, assemblyNameCopy)
  // 待优化 总觉得有更好的办法
  if (pageInfo.prop) {
...
function pageHandle (templet, config) {
  ...
  // 全局变量初始化
  [scriptData, styleText, serverTempletMap, Config, needReplaceCssList, htmlCache, srcCache, fileCache, fontList, hoverList] = [{}, '', {}, config, [], {}, [], {}, {}, new Set()]
  ...
  config.pageList.forEach(pageInfo => {
    ...
    log.debug(`页面${pageInfo.name || pageInfo.prop.name}处理用时: ${(new Date()).valueOf() - startTime} 毫秒!`)
  })
  return {
    html: templet,
    style: styleText,
