...
function handleFor (dom, scriptData) {
  ...
  if (/^[0-9]+$/.test(loopInfo["for"])) {
    ...
    // 从各种数据中找出for引用的数据
    loopInfo["for"].split('.').forEach(element => {
      ...
      for (let replaceInd = 0; replaceInd < replaceList.length; replaceInd++) {
        ...
        if (loopInfo.iterator1 === replaceValue) {
          ...
          if (typeof value === 'string') {
            console.log(`const ${loopInfo.alias} = "${value}"; return ${replaceValue}`)
            ...
            console.log(`const ${loopInfo.alias} = ${Tool.fnStringify(value)}; return ${replaceValue}`)
