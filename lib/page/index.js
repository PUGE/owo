  ...
  if (dom.attributes.length > 0) {
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      if (attribute.name === 'v-for') {
        dom.outerHTML = handleFor(dom, data)
      }
    }
  }
  // 递归处理DOM节点下面的子节点
...
// 处理模板
function bodyHandle (bodyName, htmlText, packConfig) {
...
function pageHandle (htmlText, packConfig) {
  ...
  bodyTempletArr.forEach(bodyName => {
    htmlText = bodyHandle(bodyName, htmlText, packConfig)
  })
