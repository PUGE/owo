...
function funHandle (dom, pageName, scriptData, templateName, sourceText) {
  // 模板处理
  if (dom.tagName === 'TEMPLE') {
    const templeValue = dom.attributes.getNamedItem('name').value
    logger.debug(`handle temple ${templeValue}`)
    // 处理模板
    dom.outerHTML = htmlTextHandle(pageName, dom.outerHTML, templeValue, sourceText)
    return
  }
  // a标签处理
  if (dom.tagName === 'A') {
  ...
  for (var i = 0; i < dom.children.length; i++) {
    funHandle(dom.children[i], pageName, scriptData, templateName, sourceText)
  }
}
 ...
 * @return {Object} 返回值描述
 */
function htmlTextHandle (pageName, htmlText, templateName, sourceText) {
  // 模板文件内容
  ...
  // DOM特殊标签处理
  funHandle(body, newPageName, scriptCode, newTemplateName === newPageName ? '' : newTemplateName, sourceText)
  ...
  if (body.children.length === 0) {
    ...
    // 模板替换
    newHtmlText = newHtmlText.replace(sourceText, headContent)
  }
...
function pageHandle (htmlText, packConfig) {
  ...
  // 取出所有Body标识
  const bodyTempletArr = Cut.stringArray(htmlText, '<temple', '</temple>')
  // 判断是否为单页面
  ...
  bodyTempletArr.forEach(templeText => {
    ...
    htmlText = htmlTextHandle(templetName, htmlText, templetName, `<temple${templeText}</temple>`)
  })
  // const script = Script(fnStringify(scriptData))
  // console.log(script)
  return {
    html: htmlText,
    style: styleText,
