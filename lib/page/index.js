  ...
  if (dom.attributes.length > 0) {
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      if (attribute.name === "@click") {
        // 创建属性
        dom.setAttribute('onclick', `xoClick({name: '${bodyName}', methodName: '${getAndRemoveAttr(dom, '@click')}', dom: this})`)
      } else if (attribute.name === 'v-for') {
        ...
        data[loopInfo["for"]].forEach(element => {
          let temple = dom.outerHTML
          replaceList.forEach(replaceItem => {
            const newValue = new Function(`const ${loopInfo.alias} = ${fnStringify(element)}; return ${replaceItem}`)()
            temple = temple.replace(`{{${replaceItem}}}`, newValue)
          })
          newHtml += temple
        })
