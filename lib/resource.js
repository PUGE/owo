...
class Resource {
  ...
  handleFile (config, fromPath, outPutFile, basePath, useResource) {
    if (useResource) {
      // 处理资源并移动
      log.info(`开始处理文件${fromPath}中的资源`)
      fs.readFile(fromPath, (err, fileData) => {
        if (err) throw err
        fileData = this.handle(fileData.toString(), config, basePath)
